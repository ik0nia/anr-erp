<?php
/**
 * Modul Ajutoare BPA - Banca pentru Alimente Oradea
 * Gestiune stoc Produse BPA (kg), avizuri, tabele de distributie
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sterge_gestiune_bpa'])) {
    ...
    if ($id > 0 && bpa_delete_gestiune($pdo, $id)) {
        log_activitate($pdo, "BPA: document gestiune șters ID {$id}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editeaza_gestiune_bpa'])) {
    ...
    if ($id > 0 && $nr && $data_doc && $tip && $cantitate > 0 && bpa_update_gestiune($pdo, $id, $nr, $data_doc, $tip, $cantitate, $loc ?: null, $nr_benef)) {
...
// Adăugare document (aviz sau tabel pe hârtie)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adauga_document'])) {
    ...
    if (!$tip || !$nr || !$data_doc || $cantitate <= 0) {
        $eroare = 'Completați tip document, număr, dată și cantitate (pozitivă).';
    } else {
        bpa_adauga_document($pdo, $nr, $data_doc, $tip, $cantitate, $loc ?: null, $nr_benef, $utilizator);
        if ($tip === 'aviz') {
        ...
        log_activitate($pdo, "BPA: document adăugat - {$tip} {$nr}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salveaza_tabel'])) {
    ...
    $randuri = [];
    if (!empty($_POST['randuri']) && is_array($_POST['randuri'])) {
        foreach ($_POST['randuri'] as $r) {
    ...
    if (!$nr_tabel || !$data_tabel) {
        $eroare = 'Completați numărul și data tabelului.';
    } else {
        ...
        header('Location: ajutoare-bpa.php?succes=tabel&id=' . (int)$id);
        exit;
    }
}
...
if (isset($_GET['succes'])) {
    if ($_GET['succes'] === 'document') $succes = 'Documentul a fost adăugat.';
    elseif ($_GET['succes'] === 'tabel') $succes = 'Tabelul de distributie a fost salvat. Cantitatea a fost scăzută din stoc.';
    elseif ($_GET['succes'] === 'editeaza_gestiune') $succes = 'Documentul a fost actualizat.';
    elseif ($_GET['succes'] === 'sterge_gestiune') $succes = 'Documentul a fost șters din gestiune.';
}
?>
