<?php
/**
 * Librărie documente - management documente model pentru membri
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['reordoneaza_ids']) && is_array($_POST['reordoneaza_ids'])) {
        $ids = array_map('intval', $_POST['reordoneaza_ids']);
    ...
    if (isset($_POST['incarca_document'])) {
        ...
        if (empty($institutie) || empty($nume_document)) {
            $eroare = 'Instituția și numele documentului sunt obligatorii.';
        } elseif (empty($_FILES['document']['name']) || $_FILES['document']['error'] !== UPLOAD_ERR_OK) {
            $eroare = 'Selectați un fișier valid (Word, Excel sau PDF, max. ' . LIBRARIE_DOC_MAX_MB . ' MB).';
        } else {
            $id = librarie_documente_adauga($pdo, $institutie, $nume_document, $_FILES['document']);
            if ($id > 0) {
            ...
            $eroare = 'Eroare la încărcare. Verificați formatul (Word, Excel, PDF) și mărimea (max. ' . LIBRARIE_DOC_MAX_MB . ' MB).';
        }
    }
        ...
        if ($id > 0) {
            ...
            if ($doc) {
                ...
                if ($result) {
                    ...
                    $eroare = 'Eroare la ștergerea documentului.';
                }
            } else {
                $eroare = 'Documentul nu a fost găsit.';
            }
        } else {
            $eroare = 'ID document invalid.';
        }
    }
...
<main id="main-content" class="flex-1 flex flex-col overflow-hidden" role="main">
    ...
    <div class="p-6 overflow-y-auto flex-1">
        ...
        <div class="mb-4 p-4 bg-emerald-100 dark:bg-emerald-900/30 border-l-4 border-emerald-600 text-emerald-900 dark:text-emerald-200 rounded-r" role="status" aria-live="polite">
            <?php
            $msg = ['1' => 'Document încărcat în librărie.', '2' => 'Document șters.', '3' => 'Document actualizat.'];
            echo htmlspecialchars($msg[$_GET['succes']] ?? 'Operațiune reușită.');
            ?>
        </div>
        <?php endif; ?>
