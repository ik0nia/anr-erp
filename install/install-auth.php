...
<body class="bg-gray-100 dark:bg-gray-900 min-h-screen flex items-center justify-center p-4">
    <main class="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
        ...
        <?php if ($creat): ?>
        <p class="text-emerald-700 dark:text-emerald-300 mb-4">Administrator creat. <a href="/login" class="underline font-medium">Mergi la autentificare</a>.</p>
        ...
        <p class="text-slate-700 dark:text-gray-300 mb-4">Există deja cel puțin un administrator. Folosiți <a href="/login" class="text-amber-600 underline">login</a> sau adăugați utilizatori din Setări → Management utilizatori.</p>
        <a href="/login" class="inline-block px-4 py-2 bg-amber-600 text-white rounded-lg">Autentificare</a>
        <?php else: ?>
        <?php if ($eroare): ?>
        ...
        <form method="post" action="/install/install-auth.php" class="space-y-3">
