...
$activitati_raw = [];
try {
    $rec_col = $pdo->query("SHOW COLUMNS FROM activitati LIKE 'recurenta'")->fetch();
    $sel = $rec_col ? "id, data_ora, nume, locatie, responsabili, status, info_suplimentare, recurenta" : "id, data_ora, nume, locatie, responsabili, status, info_suplimentare";
    ...
    foreach ($activitati_raw as &$a) { if (!isset($a['recurenta'])) $a['recurenta'] = null; }
} catch (PDOException $e) {
    if (strpos($e->getMessage(), "doesn't exist") !== false) {
        $eroare_bd = 'Tabelul activități nu există. Rulați schema_activitati.sql.';
    } else {
...
$activitati = expandeaza_activitati_recurente($activitati_raw, '2000-01-01', $ieri);
$activitati = array_filter($activitati, function($a) use ($azi, $an) {
    $d = (new DateTime($a['data_ora']))->format('Y-m-d');
    return $d < $azi && substr($d, 0, 4) == $an;
});
$activitati = array_values($activitati);
usort($activitati, function($a, $b) { return strcmp($b['data_ora'], $a['data_ora']); });
