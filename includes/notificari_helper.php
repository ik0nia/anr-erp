<?php
/**
 * Helper modul Notificări - transmitere informații către toți utilizatorii platformei
 */
...
/**
 * Asigură tabelele notificari și notificari_utilizatori
 */
function notificari_ensure_tables(PDO $pdo) {
...
/**
 * Număr notificări necitite pentru un utilizator
 */
...
/**
 * Returnează toți utilizatorii platformei (id, email, nume_complet) - pentru trimitere email
 */
...
/**
 * Salvează o notificare și creează înregistrări notificari_utilizatori pentru fiecare utilizator (status nou).
 * Dacă trimite_email=1, trimite email tuturor utilizatorilor.
...
function notificari_adauga(PDO $pdo, array $date, ?array $fisier_atasament = null, ?int $created_by = null): int {
    ...
    if (!empty($fisier_atasament['tmp_name']) && is_uploaded_file($fisier_atasament['tmp_name'])) {
        ...
        $upload_dir = __DIR__ . '/../uploads/notificari/';
...
/**
 * Trimite email către toți utilizatorii: subiect "Notificare ANR Bihor: [titlu]", corp: continut + link, atașament
 */
...
/**
 * Lista notificări pentru utilizatorul curent (cu excludere arhivate dacă nu se cere istoric)
 * $ordine: 'a-z' (titlu ASC) sau 'z-a' (titlu DESC)
 * $istoric: true = include și arhivate
 */
...
/**
 * O singură notificare după id (pentru utilizatorul curent)
 */
...
/**
 * Marchează notificarea ca citită pentru utilizator
 */
...
/**
 * Arhivează notificarea pentru utilizator
 */
...
/**
 * Marchează notificarea ca necitită pentru utilizator (status = 'nou')
 */
...
/**
 * Creează un task din notificare (nume = titlu notificare, detalii = link către notificare)
 */
