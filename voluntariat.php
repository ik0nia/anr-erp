<?php
/**
 * Modul Voluntariat - Nomenclator voluntari, Gestiune activități, Registru ore. WCAG 2.1.
 */
...
// —— Mesaj zilei ——
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salveaza_mesaj_voluntari'])) {
    csrf_require_valid();
    voluntariat_set_mesaj_zilei($pdo, $_POST['mesaj_voluntari'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sterge_mesaj_voluntari'])) {
    csrf_require_valid();
...
// —— Template contract (setare) ——
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['setare_template_contract'])) {
    csrf_require_valid();
    $tid = (int)($_POST['template_contract_id'] ?? 0);
    ...
    $stmt->execute([$tid ? (string)$tid : '', $tid ? (string)$tid : '']);
    header('Location: voluntariat.php?tab=nomenclator&succes=template');
    exit;
}

// —— Adaugă voluntar ——
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adauga_voluntar'])) {
    csrf_require_valid();
    $rez = voluntariat_adauga_voluntar($pdo, $_POST);
    if ($rez['success']) {
...
// —— Actualizează voluntar ——
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizeaza_voluntar'])) {
    csrf_require_valid();
    $id = (int)($_POST['voluntar_id'] ?? 0);
    if ($id > 0 && voluntariat_actualizeaza_voluntar($pdo, $id, $_POST)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adauga_activitate'])) {
    csrf_require_valid();
    $nume = trim($_POST['nume_activitate'] ?? '');
    $data_a = trim($_POST['data_activitate'] ?? '');
    $ora_i = trim($_POST['ora_inceput'] ?? '');
    $ora_s = trim($_POST['ora_sfarsit'] ?? '');
    if ($nume !== '' && $data_a !== '') {
        $aid = voluntariat_adauga_activitate($pdo, $nume, $data_a, $ora_i ?: null, $ora_s ?: null);
        if ($aid) {
            log_activitate($pdo, 'Voluntariat: Activitate adăugată - ' . $nume . ' (' . $data_a . ')');
            header('Location: voluntariat.php?tab=activitati&succes=activitate');
            exit;
        }
    }
    $eroare = 'Numele și data activității sunt obligatorii.';
}
// —— Adaugă participant la activitate ——
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adauga_participant'])) {
    csrf_require_valid();
    $aid = (int)($_POST['activitate_id'] ?? 0);
    $vid = (int)($_POST['voluntar_id'] ?? 0);
    $ore = trim($_POST['ore_prestate'] ?? '');
    if ($aid > 0 && $vid > 0 && voluntariat_adauga_participant($pdo, $aid, $vid, $ore !== '' ? (float)$ore : null)) {
    ...
    $eroare = 'Selectați activitatea și voluntarul.';
}

if (isset($_GET['succes'])) {
    $s = $_GET['succes'];
    if ($s === 'mesaj') $succes = 'Mesajul pentru voluntari a fost salvat.';
    elseif ($s === 'mesaj_sters') $succes = 'Mesajul pentru voluntari a fost șters.';
    elseif ($s === 'template') $succes = 'Template-ul pentru contract a fost setat.';
    elseif ($s === 'voluntar') $succes = 'Voluntarul a fost adăugat. Contractul a fost generat și înregistrat.';
    elseif ($s === 'actualizat') $succes = 'Voluntarul a fost actualizat.';
    elseif ($s === 'activitate') $succes = 'Activitatea a fost adăugată.';
    elseif ($s === 'participant') $succes = 'Voluntarul a fost adăugat în activitate.';
}
...
<!-- Modal Editează voluntar (formular similar, action actualizeaza_voluntar, voluntar_id) -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.editeazaVoluntar = function(id) {
        // Încarcă datele voluntarului și afișează formularul de editare (simplificat: redirect la un form cu id)
