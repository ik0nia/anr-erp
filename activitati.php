...
require_once 'includes/activitati_helper.php';
...
// Încarcă lista utilizatori pentru responsabili
$utilizatori_platforma = [];
try {
    ...
    $utilizatori_platforma = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {}

// Migrare schema: recurenta, lista_prezenta_id
try {
    $cols = $pdo->query("SHOW COLUMNS FROM activitati LIKE 'recurenta'")->fetch();
    if (!$cols) {
        $pdo->exec("ALTER TABLE activitati ADD COLUMN recurenta VARCHAR(20) DEFAULT NULL");
    }
    $cols = $pdo->query("SHOW COLUMNS FROM activitati LIKE 'lista_prezenta_id'")->fetch();
    if (!$cols) {
        $pdo->exec("ALTER TABLE activitati ADD COLUMN lista_prezenta_id INT DEFAULT NULL");
    }
} catch (PDOException $e) {}

// Procesare adăugare activitate
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adauga_activitate'])) {
    csrf_require_valid();
    $data = trim($_POST['data'] ?? '');
    $ora = trim($_POST['ora'] ?? '');
    $ora_finalizare_raw = trim($_POST['ora_finalizare'] ?? '');
    $nume = trim($_POST['nume'] ?? '');
    $locatie = trim($_POST['locatie'] ?? '');
    $responsabili_array = isset($_POST['responsabili']) && is_array($_POST['responsabili']) ? $_POST['responsabili'] : [];
    $responsabili = !empty($responsabili_array) ? implode(', ', array_filter($responsabili_array)) : '';
    $info = trim($_POST['info_suplimentare'] ?? '');
    $recurenta = $_POST['recurenta'] ?? '';

    if (empty($nume)) {
        $eroare = 'Numele activității este obligatoriu.';
    } elseif (empty($data) || empty($ora)) {
        $eroare = 'Data și ora de început sunt obligatorii.';
    } else {
        $data_ora = $data . ' ' . $ora;
        $dt = DateTime::createFromFormat('Y-m-d H:i', $data_ora);
        if (!$dt) {
            $eroare = 'Format invalid pentru data sau ora.';
        } else {
            $ora_fin_sql = null;
            if ($ora_finalizare_raw !== '') {
                $ora_fin_dt = DateTime::createFromFormat('H:i', $ora_finalizare_raw);
                if ($ora_fin_dt) {
                    $ora_fin_sql = $ora_fin_dt->format('H:i:s');
                }
            }
            try {
                $cols = $pdo->query("SHOW COLUMNS FROM activitati LIKE 'ora_finalizare'")->fetch();
                if (!$cols) {
                    $pdo->exec("ALTER TABLE activitati ADD COLUMN ora_finalizare TIME DEFAULT NULL AFTER data_ora");
                }
                $rec = in_array($recurenta, ['zilnic','saptamanal','lunar','anual']) ? $recurenta : null;
                ...
                $stmt->execute([$dt->format('Y-m-d H:i'), $ora_fin_sql, $nume, $locatie ?: null, $responsabili ?: null, $info ?: null, $rec]);
                $activitate_id = $pdo->lastInsertId();
                log_activitate($pdo, 'Activitate adăugată: ' . $nume . ' (' . $dt->format(DATETIME_FORMAT) . ')' . ($rec ? ' recurentă ' . $rec : '') . ($responsabili ? ' / Responsabili: ' . $responsabili : ''));
                $redirect = trim($_GET['redirect'] ?? $_POST['redirect'] ?? 'activitati.php');
                if (empty($redirect) || strpos($redirect, '//') !== false) {
                    $redirect = 'activitati.php';
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizeaza_status'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    $status = $_POST['status'] ?? '';
    $allowed = ['Finalizata', 'Reprogramata', 'Anulata'];
    if ($id > 0 && in_array($status, $allowed)) {
        try {
            $stmt = $pdo->prepare('SELECT nume FROM activitati WHERE id = ?');
            $stmt->execute([$id]);
            $nume = $stmt->fetchColumn();
            $stmt = $pdo->prepare('UPDATE activitati SET status = ? WHERE id = ?');
            $stmt->execute([$status, $id]);
            if ($nume) {
                log_activitate($pdo, 'Status activitate modificat: ' . $nume . ' -> ' . $status);
            }
            header('Location: activitati.php' . ($afiseaza_tot ? '?afiseaza_tot=1' : '') . '&succes_status=1');
            exit;
        } catch (PDOException $e) {
            $eroare = 'Eroare la actualizarea statusului.';
        }
    }
}
...
try {
    ...
    $stmt = $pdo->prepare("SELECT $sel FROM activitati 
                           WHERE DATE(data_ora) >= ? AND DATE(data_ora) <= ? 
                           ORDER BY data_ora ASC");
    $stmt->execute([$data_start, $data_end]);
    $activitati_raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($activitati_raw as &$a) {
        if (!isset($a['recurenta'])) $a['recurenta'] = null;
        if (!isset($a['lista_prezenta_id'])) $a['lista_prezenta_id'] = null;
    }
} catch (PDOException $e) {
    $activitati_raw = [];
    $eroare_bd = 'Eroare la încărcarea activităților.';
}

$activitati = expandeaza_activitati_recurente($activitati_raw, $data_start, $data_end);
    ...
    <div class="p-6 overflow-y-auto flex-1">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <aside class="flex flex-col gap-4" aria-label="Liste prezență și rezumat">
                ...
                $liste_prezenta = [];
                try {
                    $stmt = $pdo->query("SELECT l.id, l.tip_titlu, l.detalii_activitate, l.data_lista, l.activitate_id, l.created_by FROM liste_prezenta l ORDER BY l.data_lista DESC, l.updated_at DESC LIMIT 20");
                    $liste_prezenta = $stmt->fetchAll(PDO::FETCH_ASSOC);
                } catch (PDOException $e) {}
                ?>
